...
def send_telegram_alert(text: str) -> bool:
    """Send the Everyday-style summary, using Jenkins proxy credentials when set."""
    token = os.getenv("TELEGRAM_BOT_TOKEN", "").strip()
    chat_id = os.getenv("TELEGRAM_CHAT_ID", "").strip()
    proxy_url = os.getenv("TELEGRAM_PROXY_URL", "").strip()
    proxy_secret = os.getenv("TELEGRAM_PROXY_AUTH_SECRET", "").strip()
    ...
    if token and chat_id:
        ...
        try:
            ...
            print(f"[TELEGRAM] direct send failed: {exc}")
    if proxy_url and proxy_secret and proxy_creds:
        body = json.dumps({"title": "Business_leads_test [summary]", "text": text,
                           "creds": proxy_creds, "parse_mode": "HTML",
                           "disable_notification": False}, ensure_ascii=False).encode("utf-8")
        request = Request(proxy_url, data=body, headers={"Content-Type": "application/json", "X-Authentication": proxy_secret})
        try:
            with urlopen(request, timeout=15) as response:
                return 200 <= response.status < 300
        except Exception as exc:
            print(f"[TELEGRAM] proxy send failed: {exc}")
    print("[TELEGRAM] credentials are not configured; alert was only written to telegram_message.txt")
    return False
...
def main() -> int:
    ...
    print(message)
    if should_send and env_bool("ALERT_SEND_ENABLED", True):
