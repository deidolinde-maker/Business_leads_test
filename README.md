...
- Для МТС отдельная бизнес-форма берётся только с `https://mts-home-online.ru/business`; её контракт подготовлен к одному live-прогону, но production-заявка MTS ещё не отправлялась. Остальные MTS business-page записи исключены; checkbox / `Place=Для бизнеса` остаются отдельным скоупом. [Проверка MTS-схемы](docs/evidence/mts-business-scope-20260916.md).
