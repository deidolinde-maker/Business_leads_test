...
- Получены **262 записи для уточнения**: 16 URL бизнес-страниц и 246 страниц-кандидатов на наличие бизнес-переключателей. Это не 262 подтверждённые бизнес-формы; кандидат после осмотра может разделиться на несколько кейсов или получить доказанное исключение.
- Все импортированные записи имеют `blocked`: реальные самарские маршруты, точные формы и сетевые контракты ещё не подтверждены. Отсутствует ложный green для пустого/заблокированного live-покрытия.
- Данные пользователя сохранены: Самара, Ленинградская, 1, телефон `999999999` без добавления цифр. Окружение профиля пока `null`, поэтому реальные отправки не разрешаются.
