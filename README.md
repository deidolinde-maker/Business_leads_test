...
- Для МТС отдельная бизнес-форма берётся только с `https://mts-home-online.ru/business`. Один production-прогон прошёл: регион и бизнес-признаки совпали, получен `mail_sent`, переход `/tilda/form1/submitted`; доставка MTS в CRM ожидает подтверждения. Остальные MTS business-page записи исключены; checkbox / `Place=Для бизнеса` остаются отдельным скоупом. [Проверка MTS-схемы](docs/evidence/mts-business-scope-20260916.md).
