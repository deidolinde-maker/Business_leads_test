...
- Подключены 2 `active` кейса Билайна, остальные 264 записи `blocked`. На мобильной бизнес-странице номер из 10 цифр дал `mail_sent` и переход `/thanks`; технический итог того прогона FAIL из-за учёта служебного запроса после успеха. Исправление проверено на 16 локальных браузерных тестах; новый production-submit для него не выполнялся. [Все результаты и ограничения](docs/evidence/production-onboarding-20260916.md).
