...
- Подключены 2 `active` кейса Билайна, остальные 264 записи `blocked`. Первый production-пилот передал один запрос и получил HTTP 200, но завершился FAIL без подтверждения успеха; второй пока не выполнялся. [Наблюдения и ограничения](docs/evidence/production-onboarding-20260916.md).
