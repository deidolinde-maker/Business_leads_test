...
- Подключены 2 `active` кейса Билайна, остальные 264 записи `blocked`. Два production-пилота передали по одному запросу и завершились FAIL: первый без подтверждения успеха, второй с `validation_failed` при HTTP 200. [Наблюдения и ограничения](docs/evidence/production-onboarding-20260916.md).
- Пользователь подтвердил production и телефон `999999999`; профиль: Самара, Ленинградская, 1. Цифры сохранены буквально. Live-команда действительно отправляет заявку: не повторять первый пилот автоматически.
