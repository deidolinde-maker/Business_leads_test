...
- Подключены 2 `active` кейса Билайна; в реестре также 259 `blocked` и 5 `excluded`. На мобильной бизнес-странице номер из 10 цифр дал `mail_sent`, переход `/thanks` и подтверждённую пользователем доставку в CRM. Исправление учёта служебного запроса проверено на 16 локальных браузерных тестах; новый production-submit для него не выполнялся. [Все результаты и ограничения](docs/evidence/production-onboarding-20260916.md).
- Для МТС отдельная бизнес-форма берётся только с `https://mts-home-online.ru/business`; остальные импортированные MTS business-page записи исключены по решению пользователя. Checkbox / `Place=Для бизнеса` остаются отдельным скоупом. [Проверка MTS-схемы](docs/evidence/mts-business-scope-20260916.md).
