...
Для одного представителя семейства используйте `inspect_business_option_state.py`. Он выбирает только `Для бизнеса` и Самару в попапе, не заполняет адрес и отменяет все записи:
